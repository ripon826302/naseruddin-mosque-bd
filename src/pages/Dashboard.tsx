      ...
      <div className="relative p-6 space-y-8">
        ...
        <div className="text-center space-y-8 mb-12">
          <div className="relative">
            {/* Multiple layered glowing backgrounds for mosque name */}
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-72 h-72 bg-gradient-to-r from-red-400 via-orange-400 to-yellow-400 rounded-full opacity-20 animate-spin blur-3xl"></div>
            </div>
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-56 h-56 bg-gradient-to-r from-orange-500 via-red-500 to-pink-500 rounded-full opacity-15 animate-spin blur-2xl" style={{animationDirection: 'reverse', animationDuration: '8s'}}></div>
            </div>
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-40 h-40 bg-gradient-to-r from-yellow-400 via-orange-400 to-red-400 rounded-full opacity-25 animate-pulse blur-xl"></div>
            </div>
            
            {/* Main mosque name with red/orange gradient like maghrib */}
            <div className="relative z-10">
              <div className="mb-8">
                {/* Islamic pattern background */}
                <div className="absolute inset-0 islamic-pattern opacity-30"></div>
                
                {/* Mosque name with red/orange gradient */}
                <h1 className="relative text-5xl md:text-8xl font-bold mb-6 digital-font gradient-shift-animation"
                  style={{ 
                    background: 'linear-gradient(45deg, #ff4444, #ff8800, #ffaa00, #ff6600, #ff4444)',
                    backgroundSize: '400% 400%',
                    WebkitBackgroundClip: 'text',
                    WebkitTextFillColor: 'transparent',
                    backgroundClip: 'text',
                    filter: 'drop-shadow(0 0 30px orange) drop-shadow(0 0 60px red) drop-shadow(0 0 90px yellow)',
                    textShadow: '0 0 20px rgba(255,68,68,0.8), 0 0 40px rgba(255,136,0,0.6), 0 0 60px rgba(255,170,0,0.4)'
                  }}>
                  {settings.name}
                </h1>
                
                {/* Decorative elements around mosque name - red/orange theme */}
                <div className="absolute -top-8 left-1/4 transform -translate-x-1/2">
                  <div className="w-4 h-4 bg-red-400 rounded-full animate-bounce shadow-red-400 shadow-lg"></div>
                </div>
                <div className="absolute -top-4 right-1/4 transform translate-x-1/2">
                  <div className="w-3 h-3 bg-orange-400 rounded-full animate-bounce shadow-orange-400 shadow-lg" style={{animationDelay: '0.3s'}}></div>
                </div>
                <div className="absolute -bottom-8 left-1/3 transform -translate-x-1/2">
                  <div className="w-5 h-5 bg-yellow-400 rounded-full animate-bounce shadow-yellow-400 shadow-lg" style={{animationDelay: '0.6s'}}></div>
                </div>
                <div className="absolute -bottom-4 right-1/3 transform translate-x-1/2">
                  <div className="w-3 h-3 bg-red-300 rounded-full animate-bounce shadow-red-300 shadow-lg" style={{animationDelay: '0.9s'}}></div>
                </div>
                
                {/* Sparkle effects around title - red/orange theme */}
                <div className="absolute -top-6 left-1/6">
                  <Sparkles className="text-red-400 w-8 h-8 animate-pulse" style={{filter: 'drop-shadow(0 0 10px red)'}} />
                </div>
                <div className="absolute -top-2 right-1/6">
                  <Sparkles className="text-orange-400 w-6 h-6 animate-pulse" style={{animationDelay: '0.5s', filter: 'drop-shadow(0 0 10px orange)'}} />
                </div>
                <div className="absolute -bottom-6 left-1/5">
                  <Sparkles className="text-yellow-400 w-7 h-7 animate-pulse" style={{animationDelay: '1s', filter: 'drop-shadow(0 0 10px yellow)'}} />
                </div>
                <div className="absolute -bottom-2 right-1/5">
                  <Sparkles className="text-red-300 w-5 h-5 animate-pulse" style={{animationDelay: '1.5s', filter: 'drop-shadow(0 0 10px #fca5a5)'}} />
                </div>
              </div>
            </div>
          </div>
